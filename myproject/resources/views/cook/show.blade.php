	...
	<div class="flex-grow" style="flex:1 2 100%">
		<table class="table table-bordered table-sm mymargin5">
			<tr>
				...
				@if($row -> pic16)
					<img src="{{ asset('/storage/cook_img/'. $row->pic16) }}" width="300" heigth="250"
		...
		<table class="table table-bordered table-sm mymargin5">
			...
			<tr>
				...
				<td align="left">{{ $row->major_name }}</td>
			</tr>
			
			<tr>
				<td width="20%" class="mycolor2"> 경력</td>
				<td align="left">{{ $row->career16 }}</td>
			</tr>
		</table>
</div>

<div align="center">
    <a href="{{ route('cook.edit', $row->id) }}{{ $tmp }}" class="btn btn-sm mycolor1">수정</a>
	<form action = "{{ route('cook.destroy', $row->id)}}">
		@csrf
		@method('DELETE')
